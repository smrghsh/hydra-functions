...
function exampleTabView(state, emit) {
  ...
  if (obj !== null) {
    ...
    for (let i = 0; i < examples.length; i++) {
      ...
      const hsl = indexToHsl(
        state.page.selected.colorIndex,
        isSelected ? 100 : 20,
        isSelected ? 90 : 60
      );
      tabs.push(html`
        <div
          class="tab plex-mono pointer dib mr2 pa2 pv1"
          style="background-color:${hsl}"
          onclick=${() => emit("show details", obj, i)}
        >
          <!--${i18next.t("example")}-->
          ${i}
        </div>
    ...
    if (obj.inputs !== undefined) {
      functionName = `${obj.name}( ${obj.inputs
        .map(
          (input) =>
...
function editorView(state, emit) {
  ...
  const openin = () => {
    window.open(
      `https://hydra.ojack.xyz/?code=${btoa(
        encodeURIComponent(
          state.cache(CodeMirrorComponent, "cm-editor").getLastCode()
        )
      )}`
    );
  };
  ...
  return html`<div
    class="overflow-y-auto w-50-ns w-100 w-100-m h-100 ${obj === null
      ? "dn"
      : "db"}"
  >
    <div
      class="pa2"
      style="background-color:${indexToHsl(
        state.page.selected?.colorIndex,
        100,
        80
      )}"
    >
      <div class="flex justify-end w-100">
        <div
          class="dib pointer pa1"
          style="background-color:rgba(255,255,255,0.5)"
          onclick=${() => emit("clear details")}
        >
          ✖
        </div>
      </div>
...
function languageView(state, emit) {
  ...
  for (let i = 0; i < languages.length; i++) {
    ...
    languageButtons.push(html`
      <div class="pointer dib underline" onclick=${onclick}>
        ${i18next.getFixedT(lang)("language-name")}
      </div>
    `);
...
function functionListView(state, emit) {
  ...
  for (const group of state.hydraReference.getGroups()) {
    ...
    for (const obj of funcs) {
      ...
      const func = html`
        <div
          class="plex-mono dib ma1 token function pa1 pv1 ${obj.undocumented
            ? "gray"
            : "pointer dim"}"
          onclick=${onclick}
          title=${obj.name}
          style="border-bottom: 4px solid ${indexToHsl(
            obj.colorIndex,
            100,
            70
          )};line-height:0.6"
  ...
  return html`
    <div
      class="relative h-100 overflow-y-scroll-ns overflow-y-auto-m"
...
export default function mainView(state, emit) {
  ...
  return html`
    <body
      class="absolute pa2 f6 w-100 h-100-ns h-auto-m flex justify-center"
      style="font-family: 'Chivo', 'Noto Sans JP', sans-serif;background-color:${color};transition: background-color 1s;"
    >
        ...
        <div class="relative flex justify-between items-end mv2">
          <div class="pt2 f3">
            ...
            ${state.page.selected === null
              ? ""
              : ` ::: ${state.page.selected.name}`}
          </div>
          <div class="pv1">${languageView(state, emit)}</div>
        </div>
        <div
          class="relative flex flex-column-reverse flex-row-ns flex-column-reverse-m w-100 h-100 overflow-y-hidden overflow-x-hidden"
          style="max-width:1000px"
        >
          <div
            style=""
            class="relative flex flex-column h-100 w-50-ns w-100 w-100-m mr2"
          >
            <p class="relative">
              ${raw(
                i18next.t("intro", {
                  ...
                  gettingStarted:
                    "https://github.com/ojack/hydra#Getting-Started",
